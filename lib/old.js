...
var Rabbitr = module.exports = function constructor(opts) {
    ...
    var connect = function() {
	    ...
	    self.connection.on('ready', function () {		    	
		    ...
		    self.connection.socket.once('error', function(err) {
				self._exchangeCache = [];
				debug('rabbitr'.cyan, 'clearing exchange cache due to socket error');

		    	self.emit('error', err);
		    });
		    self.connection.socket.once('close', function() {
		    	self.emit('error', new Error('Lost connection to RabbitMQ'));
		    });
		    self.connection.socket.once('end', function() {
		    	self.emit('error', new Error('Lost connection to RabbitMQ'));
		    });	    	
	    });
...
Rabbitr.prototype._getExchange = function(exchangeName, opts, cb) {
    var self = this;

    if(self._exchangeCache[exchangeName]) {
    	// use cached exchange
		var exchange = self._exchangeCache[exchangeName];

		debug('rabbitr'.cyan, '_getExchange cached channel state is', exchange.state);
		if(exchange.state == 'open') {
			debug('rabbitr'.cyan, '_getExchange using cached channel for', exchangeName);
	        return cb(exchange);
		}

		debug('rabbitr'.cyan, '_getExchange dropping cached channel for', exchangeName, 'as state is not open');
		delete self._exchangeCache[exchangeName];
    }

    // create new exchange connection
    self.connection.exchange(exchangeName, opts, function(exchange) {
		debug('rabbitr'.cyan, '_getExchange opened new channel for', exchangeName);

		self._exchangeCache[exchangeName] = exchange;
...
Rabbitr.prototype.send = function(topic, data, cb, opts) {
	...
	self._getExchange(self.formatName(topic), opts.exchangeOpts || self.opts.exchangeOpts, function (exchange) {
	...
	self.subscribes[topic] = self.connection.queue(self.formatName(topic), self.opts.queueOpts, function (queue) {
		...
		queue.subscribe(opts || { ack: true }, function (message, headers, deliveryInfo, messageObject) {			
			...
			self.emit(topic, {
                ...
                reject: function() {
                    debug('rabbitr'.cyan, 'rejecting message', topic, message);
...
Rabbitr.prototype.bindExchangeToQueue = function(exchange, queue, cb, alreadyInQueue) {
	...
	self.connection.queue(self.formatName(queue), self.opts.queueOpts, function (q) {
		self._getExchange(self.formatName(exchange), self.opts.exchangeOpts, function (exc) {
			q.bind(exc, '#', function() {
...
Rabbitr.prototype.rpcExec = function(topic, d, cb) {
    ...
    self.rpcSubscribes[returnQueueName] = self.connection.queue(self.formatName(returnQueueName), responseQueueOpts, function (queue) {
		...
		// bind 
		self._getExchange(self.formatName(returnExchangeName), self.opts.exchangeOpts, function (exc) {
