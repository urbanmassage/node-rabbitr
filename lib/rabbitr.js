    ...
    var connect = function() {
	    ...
	    self.connection.on('ready', function () {	
	    	...
	    	var afterSetup = function() {
	    		...
	    		self.bindingsQueue = [];
	    		for(var i=0; i<self.rpcListenersQueue.length; i++) {
	    			self.rpcListener(self.rpcListenersQueue[i].topic, self.rpcListenersQueue[i].executor, true);
	    		}
	    		self.rpcListenersQueue = [];
...
Rabbitr.prototype.rpcExec = function(topic, d, cb) {
    ...
    var expiry = 40000; // 40 seconds
        
    var timeout = setTimeout(function() {
        cb({
            message: 'Request timed out'
        });
    }, expiry); // timeout after 
        
    // bind the response queue
    self.subscribe(returnQueueName, {
        "x-message-ttl": expiry + 5000 // expiry + 5s
    }, function() {
