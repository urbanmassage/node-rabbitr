...
Rabbitr.prototype.bindExchangeToQueue = function(exchange, queue, cb) {
	...
	self.connection.queue(self.formatName(queue), self.queueOpts, function (q) {
			
		self.connection.exchange(self.formatName(exchange), self.exchangeOpts, function (exc) {
			q.bind(exc, '#', function() {
...
Rabbitr.prototype.setTimer = function(topic, uniqueID, data, ttl, cb) {
    ...
    self.connection.queue(self.formatName(timerQueue), {
        arguments: {
          "x-dead-letter-exchange": environmentTag + topic, 
          "x-message-ttl": ttl, 
          "x-expires": (ttl + 1000)
        }
    }, function (dlq) {
...
Rabbitr.prototype.rpcExec = function(topic, d, cb) {
    ...
    console.log('doing rpcExec');
    
    // bind the response queue
    self.subscribe(returnQueueName, function() {
	    self.bindExchangeToQueue(returnQueueName, returnQueueName, function() {
...
Rabbitr.prototype.rpcListener = function(topic, executor) {
    ...
    self.bindExchangeToQueue(rpcQueue, rpcQueue);
